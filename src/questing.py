...
class Quester:
    ...
    async def heal_and_handle_potions(self):
        ...
        if any_client_needs_potions:
            ...
            await asyncio.gather(
                *[self.gather_clients_from_potion_buy(c) for c in self.clients]
            )
    ...
    async def auto_collect_rewrite(self, client: Client):
        ...
        for points in chunk_cords:  # loops through the points
            points = XYZ(
                points.x, points.y, points.z - 550
            )  # sets cord to underground to avoid pull / detection
            await client.teleport(points)  # teleports under the area

            entities = (
                await self.client.get_base_entity_list()
            )  # gets the entity list of the map
            for e in entities:
                try:
                    object_template = await e.object_template()  # gets entity template
                    display_name_code = (
                        await object_template.display_name()
                    )  # gets display name code
                    ...
                    if (
                        match > 80
                    ):  # if strings match greater than 80 it means that it's most likely the item
                        while (
    ...
    async def collect_entity(self, e: DynamicClientObject) -> bool:
        ...
        for _ in range(2):  # try's to collect item twice if not safe
            ...
            if can_Teleport:
                ...
                if await is_visible_by_path(
                    self.client, path=npc_range_path
                ):  # checks if there is an UI
                    for i in range(5):
                        await asyncio.gather(
    ...
    async def auto_quest_leader(
        ...
        while self.client.questing_status:
            ...
            follower_clients, client_quests = (
            ...
            if await is_free_leader_questing(self.current_leader_client):
                ...
                if distance > 1:
                    ...
                    if distance < 1:
                        quest_objective = await get_quest_name(
                            self.current_leader_client
                        )
