...
async def wait_for_coro(
    coro: Coroutine, wait_for_not: bool = False, interval: float = 0.25
):
...
async def parse_command(clients: List[Client], command_str: str):
    ...
    match split_command[0].lower():
        ...
        case "log" | "debug" | "print":
            ...
            if (
                len(split_command) >= 3
                and split_command[1].lower() == "window"
                and type(split_command[2]) == list
            ):
                for client in clients:
                    desired_window = await get_window_from_path(
                        client.root_window, split_command[2]
                    )
        ...
        case _:
            ...
            if "mass" not in client_str:
                ...
                if ":" in client_str:
                    ...
                    provided_clients = [
                        client_from_titles(all_clients.copy(), title)
                        for title in split_clients
                    ]
                else:
                ...
                if is_numeric(client_str[1]):
                    if exclude:
                        ...
                        clients = [
                            ...
                            if client not in provided_clients
                        ]
                    else:
            ...
            match split_command[1].lower():
                    ...
                    match split_command[2]:
                        case "closestmob" | "mob":
                            await asyncio.gather(
                                *[SprintyClient(p).tp_to_closest_mob() for p in clients]
                            )
                        ...
                        case "quest" | "questpos" | "questposition":
                            await asyncio.gather(
                                *[
                                    p.teleport(
                                        await clients[0].quest_position.position()
                                    )
                                    for p in clients
                                ]
                            )
                        ...
                        case _:
                            ...
                            for p in all_clients:
                                if p.title == split_command[2]:
                                    ...
                                    await asyncio.gather(
                                        *[
                                            client.teleport(client_location)
                                            for client in clients
                                        ]
                                    )
                                    break
                            ...
                            if client_location is None:
                                xyzs = []
                                for client in clients:
                                    client_xyzs, _ = await parse_location(
                                        split_command, client=client
                                    )
                                ...
                                await asyncio.gather(
                                    *[
                                        client.teleport(xyz)
                                        for client, xyz in zip(clients, xyzs)
                                    ]
                                )
                ...
                case "sendkey" | "press" | "presskey":
                    ...
                    await asyncio.gather(
                        *[client.send_key(Keycode[key], time) for client in clients]
                    )

                case "waitfordialog" | "waitfordialogue":
                    # Waits for dialogue window to appear
                    await asyncio.gather(
                        *[wait_for_coro(client.is_in_dialog) for client in clients]
                    )
                    ...
                    if split_command[1].lower() == "completion":
                        ...
                        await asyncio.gather(
                            *[
                                wait_for_coro(client.is_in_dialog, True)
                                for client in clients
                            ]
                        )
                ...
                case "waitforbattle" | "waitforcombat":
                    # Waits for combat
                    await asyncio.gather(
                        *[wait_for_coro(client.in_battle) for client in clients]
                    )
                    ...
                    if split_command[-1].lower() == "completion":
                        ...
                        await asyncio.gather(
                            *[
                                wait_for_coro(client.in_battle, True)
                                for client in clients
                            ]
                        )
                ...
                case "waitforzonechange" | "wait_for_zone_change":
                    ...
                    if split_command[-2].lower() == "from":
                        await asyncio.gather(
                            *[
                                wait_for_zone_change(
                                    client, current_zone=split_command[-1]
                                )
                                for client in clients
                            ]
                        )
                    # waits for zone to change to the provided zone name
                        ...
                        await asyncio.gather(
                            *[
                                wait_for_zone_change(client, to_zone=split_command[-1])
                                for client in clients
                            ]
                        )
                    else:
                        ...
                        await asyncio.gather(
                            *[client.wait_for_zone_change() for client in clients]
                        )
                        ...
                        if split_command[-1].lower() == "completion":
                            ...
                            await asyncio.gather(
                                *[
                                    wait_for_coro(client.is_loading, True)
                                    for client in clients
                                ]
                            )
                ...
                case "waitforfree":
                    ...
                    async def _wait_for_free(
                        client: Client,
                        wait_for_not: bool = False,
                        interval: float = 0.25,
                    ):
                        if wait_for_not:
                    ...
                    await asyncio.gather(
                        *[_wait_for_free(client) for client in clients]
                    )

                    if split_command[-1].lower() == "completion":
                        # Waits for is_free to return False
                        await asyncio.gather(
                            *[_wait_for_free(client, True) for client in clients]
                        )
                ...
                case "usepotion":
                    ...
                    if len(split_command) > 3:
                        ...
                        await asyncio.gather(
                            *[use_potion(client) for client in clients]
                        )
                    ...
                    if len(split_command) > 2:
                        ...
                        if split_command[2] == "ifneeded":
                            results = await asyncio.gather(
                                *[
                                    refill_potions_if_needed(client)
                                    for client in clients
                                ]
                            )
                    else:
                ...
                case "click":
                    ...
                    async def command_parser_click_mouse_handler(client):
                        async with client.mouse_handler:
                            await client.mouse_handler.click(
                                int(split_command[2], int(split_command[3]))
                            )

                    await asyncio.gather(
                        *[
                            command_parser_click_mouse_handler(client)
                            for client in clients
                        ]
                    )
                ...
                case "clickwindow":
                    ...
                    await asyncio.gather(
                        *[
                            click_window_by_path(client, split_command[2], True)
                            for client in clients
                        ]
                    )
                ...
                case "waitforwindow" | "waitforpath":
                    ...
                    await asyncio.gather(
                        *[
                            wait_for_visible_by_path(client, split_command[2])
                            for client in clients
                        ]
                    )
                    if (
                        type(split_command[-1]) == str
                        and split_command[-1].lower() == "completion"
                    ):
                        # Waits for a specific window (by path) to not be visible
                        await asyncio.gather(
                            *[
                                wait_for_visible_by_path(client, split_command[2], True)
                                for client in clients
                            ]
                        )
                ...
                case "friendtp" | "friendteleport":
                    ...
                    if split_command[2] == "icon":
                        # uses fish icon
                        async def teleport_to_friend_from_list_fish_icon_mouse_handler(
                            client,
                        ):
                        ...
                        await asyncio.gather(
                            *[
                                teleport_to_friend_from_list_fish_icon_mouse_handler(
                                    client
                                )
                                for client in clients
                            ]
                        ...
                        async def teleport_to_friend_from_list_wizard_name_mouse_handler(
                            client,
                        ):
                            async with client.mouse_handler:
                                await teleport_to_friend_from_list(
                                    client, name=" ".join(split_command[2:])
                                )

                        await asyncio.gather(
                            *[
                                teleport_to_friend_from_list_wizard_name_mouse_handler(
                                    client
                                )
                                for client in clients
                            ]
                ...
                case "entitytp" | "entityteleport":
                    ...
                    await asyncio.gather(
                        *[
                            SprintyClient(client).tp_to_closest_by_vague_name(
                                split_command[2]
                            )
                            for client in clients
                        ]
                ...
                case "tozone" | "to_zone":
                    ...
                    if zoneChanged == 0:
                        logger.debug(
                            "Reached destination zone: " + await clients[0].zone_name()
                        )
                    else:
                        logger.error(
    ...
    match split_command[0].lower():
        ...
        case "rotatingglideto":
            ...
            await rotating_glide_to(
                camera, origin_pos, handle_index(xyzs), time, handle_index(orientations)
            )
