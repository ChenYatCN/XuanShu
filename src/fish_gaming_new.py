...
from wizwalker import Client, ClientHandler, Keycode
...
async def patch(client: Client) -> List[tuple[int, bytes]]:
    async def readbytes_writebytes(
        ...
        add = (
            await reader.pattern_scan(
                pattern, return_multiple=False, module="WizardGraphicalClient.exe"
            )
            + offset
        )
    ...
    async def instant_fish():
        ...
        write_bytes = b"\x90" * num_nops
        pattern = (
            rb"\x74\x63\x48\x8B\xCF\xE8....\x0F"  # 74 63 48 8B CF E8 ?? ?? ?? ?? 0F
        )
        address_oldbytes.append(await readbytes_writebytes(pattern, write_bytes))
    ...
    async def instant_fish_3():
        ...
        write_bytes = b"\x90" * num_nops
        pattern = (
            rb"\x0F\x86....\xF3\x41\x0F\x5C\xF2"  # 0F 86 ?? ?? ?? ?? F3 41 0F 5C F2
        )
        address_oldbytes.append(await readbytes_writebytes(pattern, write_bytes))
    ...
    async def skip_summon_animation():
        ...
        address_oldbytes.append(
            await readbytes_writebytes(pattern, write_bytes, offset=7)
        )
    ...
    async def skip_caught_fish_window():
        ...
        address_oldbytes.append(
            await readbytes_writebytes(pattern, write_bytes, offset=9)
        )

    async def zero_casting_timer():
        write_bytes = b"\x00\x00\x00\x00"
        pattern = (
            rb"\x49\x8D.\xD0\x00\x00\x00\x48\x8B\x01\xBA\x14\x05\x00\x00\xFF\x50\x18"
        )
        address_oldbytes.append(
            await readbytes_writebytes(pattern, write_bytes, offset=11)
        )

    async def zero_summon_timer():
        write_bytes = b"\x00\x00\x00\x00"
        pattern = (
            rb"\x48\x8D.\xA0\x00\x00\x00\x48\x8B\x01\xBA\x14\x05\x00\x00\xFF\x50\x18"
        )
        address_oldbytes.append(
            await readbytes_writebytes(pattern, write_bytes, offset=11)
        )
...
async def wait_for_window(client, window_name, *, timeout=10, check_if_visible=True):
    start = time()
    while not await window_exists(
        client, window_name, check_if_visible=check_if_visible
    ):
        if time() - start >= timeout:
            break


async def wait_to_click_window_with_name(
    client: Client, window_name: str, *, timeout=10, check_if_visible=True
):
    await wait_for_window(
        client, window_name, timeout=timeout, check_if_visible=check_if_visible
    )
...
async def main():
    ...
    try:
        ...
        while True:
            ...
            print(
                f"Fish Caught: {fish_caught}, Number of fish in pool: {len(fish_list) - 1}, Time: {total_time} minutes, Seconds per fish: {round((total_time / fish_caught) * 60, 2)}"
            )
