...
from src.gui.helpers import (
    build_shared_svgs,
    configure_titlebar_button, titlebar_button_style, titlebar_control_svg,
    centered_label,
    copy_callback,
    copy_icon_btn,
    init_recent_imports,
...
def manage_gui(
    ...
    _tc = _text_color
    _sc = _stroke_color
    _close_svg = titlebar_control_svg(_sc, close=True)
    _minimize_svg = titlebar_control_svg(_sc)
    ...
    _titlebar_btn_style = titlebar_button_style()
    _close_btn_style = titlebar_button_style(close=True)
    ...
    app_icon_label.setFixedSize(24, 24)
    app_icon_label.setStyleSheet("background: transparent;")

    _icon_path = get_current_icon_path()
    ...
    titlebar_layout.addWidget(app_icon_label)
    ...
    configure_titlebar_button(minimize_btn, _titlebar_svg_icon, _sc)
    ...
    configure_titlebar_button(close_btn, _titlebar_svg_icon, _sc, close=True)
    ...
    ctx.app_icon_label = app_icon_label
    ...
    footer_vbox.addLayout(
        _footer_row(
            zone_label, copy_icon_btn(ctx, copy_callback(send_queue, GUIKeys.copy_zone))
        )
    )
    ...
    footer_vbox.addLayout(
        _footer_row(
            xyz_label,
            copy_icon_btn(ctx, copy_callback(send_queue, GUIKeys.copy_position)),
        )
    )
    ...
    footer_vbox.addLayout(
        _footer_row(
            pry_label,
            copy_icon_btn(ctx, copy_callback(send_queue, GUIKeys.copy_rotation)),
        )
    )
                            ...
                            if _swapped[0]:
                                ...
                                for (side, idx), info in value.items():
                                    ...
                                    if "is_friendly" in new_info:
                                        new_info["is_friendly"] = not new_info[
                                            "is_friendly"
                                        ]
