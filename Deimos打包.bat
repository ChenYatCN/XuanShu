...
if not exist "Deimos.py" (
    echo [错误] 当前目录未找到 Deimos.py
    echo 请把本 bat 放在 Deimos.py 同级目录。
    pause
    exit /b 1
)

if not exist "Deimos.spec" (
    echo [错误] 当前目录未找到 Deimos.spec
    echo 请确认 spec 文件名是否为 Deimos.spec。
    pause
    exit /b 1
)
...
".venv\Scripts\python.exe" -m PyInstaller "Deimos.spec" --clean -y
